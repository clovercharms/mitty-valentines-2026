...
func play_sound(sound: AudioStream, volume: float = 0.66):
    var audioPlayer: AudioStreamPlayer = AudioStreamPlayer.new()
    audioPlayer.stream = sound
    audioPlayer.volume_linear = volume
    GameBase.get_singleton().call_deferred("add_child", audioPlayer)
    audioPlayer.call_deferred("play")
    await audioPlayer.finished
    audioPlayer.queue_free()

func play_sound_at_2d(sound: AudioStream, location: Vector2, volume: float = 0.66):
